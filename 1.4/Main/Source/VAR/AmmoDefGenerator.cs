...
namespace VAR
{
	...
	public static class AmmoDefGenerator
    {
			...
			foreach (AmmoDefTemplate allDef in DefDatabase<AmmoDefTemplate>.AllDefs)
			{
				...
				foreach (AmmoDefPrimary primary in DefDatabase<AmmoDefPrimary>.AllDefs)
				{
					...
					foreach (AmmoDefSecondary secondary in DefDatabase<AmmoDefSecondary>.AllDefs)
					{
						...
						thingDef.defName = primary.defName + "_" + secondary.defName + "_" + thingDef.defName +"_ammo";
						...
						if (primary.modExtensions != null)
						{
							thingDef.modExtensions.AddRange(primary.modExtensions);
						}
						if (secondary.modExtensions != null)
						{
							thingDef.modExtensions.AddRange(secondary.modExtensions);
						}
						float num = 0f;
		...
		public static ThingDef BaseAmmo(AmmoDefTemplate def)
		{
			...
			{
				thingDef.modExtensions.AddRange(def.modExtensions);
			}
			//Log.Message("log14");
